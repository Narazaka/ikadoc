...
anchornotselectstyle等でstyleのみを変更した場合、anchornotselectfontcolor等を設定していない場合は全てrgb(0,0,0)となる。
デフォルトのanchorfontcolor等を使う挙動もまた合理的であるが、SSPの挙動に準拠してこちらの挙動を採用している。
